...
def attn_feature(input_feature, output_dim, neighbors_idx, activation, in_dropout=0.0, coef_dropout=0.0, is_training=None, bn_decay=None, layer='', k=20, i=0, is_dist=False):
    ...
    neighbors = tf_util.get_neighbors(input_feature, nn_idx=neighbors_idx, k=k)  # 得到邻居(包括自己)
    input_feature_tiled = tf.tile(input_feature, [1, 1, k, 1])                   # 中心点复制k份
    edge_feature = input_feature_tiled - neighbors                               # 得到了所有的边，包括自己指向自己
